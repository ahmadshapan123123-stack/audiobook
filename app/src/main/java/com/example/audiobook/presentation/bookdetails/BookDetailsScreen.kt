...
fun BookDetailsScreen(
    ...
    Column(modifier = Modifier.fillMaxSize().verticalScroll(rememberScrollState()).padding(AppSpacing.lg), verticalArrangement = Arrangement.spacedBy(AppSpacing.md)) {
        Row(modifier = Modifier.fillMaxWidth(), verticalAlignment = Alignment.CenterVertically) {
            TextButton(onClick = onBack, modifier = Modifier.minTouchTarget()) { Text("رجوع") }
            Text("مركز إدارة الكتاب", style = MaterialTheme.typography.titleLarge, color = MaterialTheme.colorScheme.onSurface, modifier = Modifier.weight(1f).padding(start = AppSpacing.sm))
        }
        ...
        Button(onClick = { uiState.defaultEditionId?.let { onPlay(it) } }, modifier = Modifier.minTouchTarget()) { Text("تشغيل الإصدار الافتراضي") }

        Row(horizontalArrangement = Arrangement.spacedBy(AppSpacing.xs)) {
            FilterChip(activeTab == "overview", { activeTab = "overview" }, label = { Text("البيانات") }, modifier = Modifier.minTouchTarget())
            FilterChip(activeTab == "editions", { activeTab = "editions" }, label = { Text("الإصدارات") }, modifier = Modifier.minTouchTarget())
            FilterChip(activeTab == "content", { activeTab = "content" }, label = { Text("المحتوى") }, modifier = Modifier.minTouchTarget())
        }
        ...
        when (activeTab) {
            "overview" -> {
                ...
                }, modifier = Modifier.minTouchTarget()) { Text("حفظ البيانات") }
                SectionTitle("الغلاف")
                Text("${book.coverImagePath ?: "لا يوجد"} · المصدر: ${book.coverSource}")
                Row(horizontalArrangement = Arrangement.spacedBy(AppSpacing.xs)) {
                    Button(onClick = { viewModel.setUserCover(book, "user-selected-cover.jpg") }, modifier = Modifier.minTouchTarget()) { Text("اختيار غلاف يدوي") }
                    TextButton(onClick = {
                    ...
                    }, modifier = Modifier.minTouchTarget()) { Text("إعادة الاكتشاف") }
                }
            ...
            "content" -> {
                ...
                uiState.defaultEditionId?.let { editionId ->
                    Button(onClick = { onBookmarks(editionId) }, modifier = Modifier.minTouchTarget()) { Text("فتح شاشة Bookmarks") }
                }
    ...
    editions.forEach { edition ->
        ...
        Card(colors = CardDefaults.cardColors(containerColor = MaterialTheme.colorScheme.surfaceVariant)) {
            Column(modifier = Modifier.padding(AppSpacing.md), verticalArrangement = Arrangement.spacedBy(AppSpacing.xs)) {
                ...
                Row(horizontalArrangement = Arrangement.spacedBy(AppSpacing.xs)) {
                    TextButton(onClick = { viewModel.renameEdition(edition, label) }, modifier = Modifier.minTouchTarget()) { Text("إعادة تسمية") }
                    TextButton(onClick = { viewModel.changeNarrator(edition, narrator) }, modifier = Modifier.minTouchTarget()) { Text("تغيير الراوي") }
                    TextButton(onClick = { viewModel.setDefaultEdition(edition.id) }, modifier = Modifier.minTouchTarget()) { Text(if (defaultId == edition.id) "افتراضي ✓" else "تعيين افتراضي") }
                }
                Row(horizontalArrangement = Arrangement.spacedBy(AppSpacing.xs)) {
                    TextButton(onClick = { viewModel.deleteEdition(edition) }, modifier = Modifier.minTouchTarget()) { Text("فصل") }
                    TextButton(onClick = { viewModel.splitEdition(edition) }, modifier = Modifier.minTouchTarget()) { Text("إنشاء نسخة") }
                }
            }
        }
    }
