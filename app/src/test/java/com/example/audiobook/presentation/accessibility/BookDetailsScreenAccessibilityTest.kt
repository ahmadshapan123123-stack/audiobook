...
class BookDetailsScreenAccessibilityTest {
    ...
    private fun showBookDetails(fontScale: Float = 1f) {
        ...
        composeRule.waitUntil(5_000) {
            composeRule.onAllNodesWithText("تفاصيل الكتاب").fetchSemanticsNodes().isNotEmpty()
        }
    }
    ...
    fun interactiveTargetsMeetMin48DpAcrossAllTabs() {
        ...
        val backButton = composeRule.onNodeWithContentDescription("رجوع").getUnclippedBoundsInRoot()
        assertTrue("رجوع height < 48dp", backButton.bottom - backButton.top >= 47.9.dp)

        assertMinTouchTarget("تشغيل", "height")
        assertMinTouchTarget("البيانات", "height")
        assertMinTouchTarget("الإصدارات (1)", "height")
        assertMinTouchTarget("المحتوى", "height")

        composeRule.onNode(hasScrollAction()).performScrollToNode(hasText("اختيار غلاف يدوي"))
        composeRule.waitForIdle()
        assertMinTouchTarget("اختيار غلاف يدوي", "height")
        composeRule.onNode(hasScrollAction()).performScrollToNode(hasText("إعادة الاكتشاف"))
        composeRule.waitForIdle()
        assertMinTouchTarget("إعادة الاكتشاف", "height")

        composeRule.onNode(hasScrollAction()).performScrollToNode(hasText("الإصدارات (1)"))
        composeRule.onNodeWithText("الإصدارات (1)").performClick()
        composeRule.waitForIdle()
        composeRule.onNode(hasScrollAction()).performScrollToNode(hasText("الافتراضي"))
        composeRule.waitForIdle()
        assertMinTouchTarget("الافتراضي", "height")
        composeRule.onNode(hasScrollAction()).performScrollToNode(hasText("حذف"))
        composeRule.waitForIdle()
        assertMinTouchTarget("حذف", "height")
        composeRule.onNode(hasScrollAction()).performScrollToNode(hasText("إنشاء نسخة"))
        composeRule.waitForIdle()
        assertMinTouchTarget("إنشاء نسخة", "height")

        composeRule.onNode(hasScrollAction()).performScrollToNode(hasText("المحتوى"))
        composeRule.onNodeWithText("المحتوى").performClick()
        composeRule.waitForIdle()
        composeRule.onNode(hasScrollAction()).performScrollToNode(hasText("الفصل الأول"))
        composeRule.waitForIdle()
        assertMinTouchTarget("الفصل الأول", "height")
        composeRule.onNode(hasScrollAction()).performScrollToNode(hasText("فتح شاشة الإشارات"))
        composeRule.waitForIdle()
        assertMinTouchTarget("فتح شاشة الإشارات", "height")
    }
    ...
    fun tabChipsSwitchContentAndRevealRelatedElements() {
        ...
        composeRule.onNodeWithText("الإصدارات (1)").performClick()
        composeRule.waitForIdle()
        composeRule.onAllNodesWithText("تعيين افتراضي").assertCountEquals(1)
        composeRule.onAllNodesWithText("الافتراضي").assertCountEquals(1)
        composeRule.onAllNodesWithText("حذف").assertCountEquals(1)
        composeRule.onAllNodesWithText("إنشاء نسخة").assertCountEquals(1)
        composeRule.onAllNodesWithText("فتح شاشة الإشارات").assertCountEquals(0)

        composeRule.onNodeWithText("المحتوى").performClick()
        composeRule.waitForIdle()
        composeRule.onAllNodesWithText("الفصول (1)").assertCountEquals(1)
        composeRule.onAllNodesWithText("الإشارات والملاحظات (1)").assertCountEquals(1)
        composeRule.onAllNodesWithText("فتح شاشة الإشارات").assertCountEquals(1)
    }
    ...
    fun enlargedTextKeepsTitleInsideScreenWithoutClipping() {
        ...
        val backButton = composeRule.onNodeWithContentDescription("رجوع").getUnclippedBoundsInRoot()
        val headerTitle = composeRule.onAllNodesWithText("تفاصيل الكتاب").onFirst().getUnclippedBoundsInRoot()
