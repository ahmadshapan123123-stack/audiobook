...
class StatisticsScreenAccessibilityTest {
    ...
    private fun showScreen(fontScale: Float = 1f) {
        ...
        composeRule.waitUntil(5_000) {
            composeRule.onAllNodesWithText("إحصائياتك").fetchSemanticsNodes().isNotEmpty()
        }
    }
    ...
    fun statsReportRealSeededData() {
        ...
        composeRule.waitUntil(5_000) {
            composeRule.onAllNodesWithText("25 دقيقة").fetchSemanticsNodes().isNotEmpty()
        }

        composeRule.onAllNodesWithText("إحصائياتك").assertCountEquals(1)
        composeRule.onAllNodesWithText("وقت الاستماع").assertCountEquals(1)
        composeRule.onAllNodesWithText("25 دقيقة").assertCountEquals(1)
        composeRule.onAllNodesWithText("هذا الأسبوع").assertCountEquals(2)
        composeRule.onAllNodesWithText("جلسات").assertCountEquals(1)
        composeRule.onAllNodesWithText("كتب").assertCountEquals(1)
        composeRule.onAllNodesWithText("أيام").assertCountEquals(1)
        composeRule.onAllNodesWithText("أسطورة نوماد").assertCountEquals(2)
        composeRule.onAllNodesWithText("استماعك يومًا بيوم").assertCountEquals(1)
        composeRule.onAllNodesWithText("عادات الاستماع").assertCountEquals(1)
        composeRule.onAllNodesWithText("سرعة الاستماع").assertCountEquals(1)
        composeRule.onAllNodesWithText("سجل الاستماع").assertCountEquals(1)
    }
    ...
    fun enlargedTextKeepsHeaderInsideScreenWithoutClipping() {
        ...
        for (text in listOf("رجوع", "إحصائياتك", "نظرة على وقتك مع الحكايات.")) {
