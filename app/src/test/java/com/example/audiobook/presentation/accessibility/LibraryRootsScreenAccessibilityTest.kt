...
class LibraryRootsScreenAccessibilityTest {
    ...
    fun switchRowsExposeLabeledToggleSemantics() {
        ...
        for (cd in listOf(
            "مفتاح الأولوية لـ Sounds",
            "مفتاح التمكين لـ Sounds",
            "مفتاح الأولوية لـ Podcasts",
            "مفتاح التمكين لـ Podcasts"
        )) {
            composeRule.onAllNodes(
                useUnmergedTree = false,
    ...
    fun togglingPriorityReflectsInSemanticsAndDatabase() {
        ...
        val podcastsPriority = labeledToggle("مفتاح الأولوية لـ Podcasts")
        podcastsPriority.assertIsOff()
        podcastsPriority.performClick()
        composeRule.waitUntil(5_000) {
            composeRule.onAllNodes(
                useUnmergedTree = false,
                matcher = hasContentDescription("مفتاح الأولوية لـ Podcasts") and hasClickAction()
            ).onFirst().let { node ->
                runCatching { node.assertIsOn() }.isSuccess
            }
        }
        labeledToggle("مفتاح الأولوية لـ Podcasts").assertIsOn()
    ...
    fun controlsMeetMin48Dp() {
        ...
        val addBounds = composeRule.onNodeWithText("إضافة مجلد").getUnclippedBoundsInRoot()
        assertTrue("'إضافة مجلد' height (${addBounds.bottom - addBounds.top}) < 48dp", addBounds.bottom - addBounds.top >= 47.9.dp)
        val refreshBounds = composeRule.onAllNodesWithText("تحديث").onFirst().getUnclippedBoundsInRoot()
        assertTrue("'تحديث' height (${refreshBounds.bottom - refreshBounds.top}) < 48dp", refreshBounds.bottom - refreshBounds.top >= 47.9.dp)
        for (cd in listOf(
            "مفتاح الأولوية لـ Sounds",
            "مفتاح التمكين لـ Sounds",
            "مفتاح الأولوية لـ Podcasts",
    ...
    fun enlargedTextKeepsLabelsAndRowsInsideScreenWithoutClipping() {
        ...
        for (text in listOf("مجلدات المكتبة", "إضافة مجلد", "Sounds", "أولوية", "ممكّن", "تحديث")) {
