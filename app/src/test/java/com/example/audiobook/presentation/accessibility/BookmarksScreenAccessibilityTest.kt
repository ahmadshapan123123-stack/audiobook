...
class BookmarksScreenAccessibilityTest {
    ...
    private fun showBookmarks(fontScale: Float = 1f) {
        ...
        composeRule.waitUntil(5_000) {
            composeRule.onAllNodesWithText("عند 02:00 · إشارة").fetchSemanticsNodes().isNotEmpty()
        }
    }
    ...
    private fun assertIconTouchTargetHeight(contentDescription: String) {
        val bounds = composeRule.onNodeWithContentDescription(contentDescription).getUnclippedBoundsInRoot()
        val actual = bounds.bottom - bounds.top
        assertTrue("'$contentDescription' height ($actual) < 48dp", actual >= 47.9.dp)
    }
    ...
    fun bookmarkActionsMeetMin48Dp() {
        ...
        assertIconTouchTargetHeight("رجوع")
    ...
    fun seekTargetMovesPlaybackAndDeleteRemovesRowFromSemantics() {
        ...
        composeRule.onAllNodesWithText("عند 02:00 · إشارة").assertCountEquals(1)
        ...
        composeRule.waitUntil(5_000) {
            composeRule.onAllNodesWithText("عند 02:00 · إشارة").fetchSemanticsNodes().isEmpty()
        }
        composeRule.onAllNodesWithText("عند 02:00 · إشارة").assertCountEquals(0)
    }
    ...
    fun enlargedTextKeepsHeaderInsideScreenWithoutClipping() {
        ...
        val backBounds = composeRule.onNodeWithContentDescription("رجوع").getUnclippedBoundsInRoot()
        assertTrue("'رجوع' يعبر الحافة اليمنى", backBounds.right <= root.right)
        assertTrue("'رجوع' يعبر الحافة اليسرى", backBounds.left >= root.left)
        for (text in listOf("الإشارات والملاحظات", "عند 02:00 · إشارة")) {
